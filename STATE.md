...
5. **Cross-organisation work — designed but not scheduled.** Recorded in `PRODUCT.md` under Users and "Work that crosses organisations". The cheap half is decided (dated assignment, above); the rest is open: does a ward scope sit above organisation scope, is ward council a meeting type, how is a bishopric seat scoped across workspaces rather than across areas, and what guardrail stops pastoral content entering at the point where it is most likely to. **No schema beyond `workspaces.organization` has been built for this.**
